...
int main(int argc, char** argv) {
    ...
    std::cout << "=== MoE FPGA v3 Testbench (INT16 gate, Opt 4 — fused W1+W2/W3+W2) ===" << std::endl;
    ...
    // ------------------------------------------------------------------------
    // Symmetric zero-centered quantization for x
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // Symmetric zero-centered quantization for W_gate
    // ------------------------------------------------------------------------
    ...
    // Opt 3: split W_gate across two HBM ports.
    //   w_gate_mem   : experts [0..N_EXPERTS_TOTAL/2)
    //   w_gate_mem_b : experts [N_EXPERTS_TOTAL/2..N_EXPERTS_TOTAL)
    ...
    // ------------------------------------------------------------------------
    // Opt 5: per-expert layout on each fused port (3-way row interleaving a/b/c):
    //   W1_W2a port: [ W1 rows where row%3==0 (W1_THIRD_VECS) |
    //                  W2 first-half  rows where row%3==0 (W2_SIXTH_VECS) ]
    //   W1_W2b port: [ W1 rows where row%3==1 ... | W2 first-half  rows where row%3==1 ... ]
    //   W1_W2c port: [ W1 rows where row%3==2 ... | W2 first-half  rows where row%3==2 ... ]
    //   W3_W2{a,b,c}: same scheme for W3 / W2 second-half.
    // The c-port is the same size as a/b (683 W1 rows, 1195 W2-half rows). The
    // last group on the c-port is dummy padding (row 2048 for W1, row 3584 for
    // W2-half) — fill with zeros so sum_c = 0 in the kernel and host can crop.
    // ------------------------------------------------------------------------
    aligned_vector<int16_vec_t> W1_W2a_src((size_t)N_TEST_EXPERTS * W1_W2C_PORT_VECS);
    aligned_vector<int16_vec_t> W1_W2b_src((size_t)N_TEST_EXPERTS * W1_W2C_PORT_VECS);
    aligned_vector<int16_vec_t> W1_W2c_src((size_t)N_TEST_EXPERTS * W1_W2C_PORT_VECS);
    aligned_vector<int16_vec_t> W3_W2a_src((size_t)N_TEST_EXPERTS * W3_W2C_PORT_VECS);
    aligned_vector<int16_vec_t> W3_W2b_src((size_t)N_TEST_EXPERTS * W3_W2C_PORT_VECS);
    aligned_vector<int16_vec_t> W3_W2c_src((size_t)N_TEST_EXPERTS * W3_W2C_PORT_VECS);

    // Helper lambda: zero-init a vec.
    auto zero_vec = []() {
        int16_vec_t z;
    ...
    // Recompute the row counts the kernel expects on each port.
    // For backward compatibility the a/b ports keep the legacy "even/odd" layout
    // but reorganize as row%3==0 (a) and row%3==1 (b). The kernel reads them in
    // 3-way groups (a, b, c) so we must pack them in that order.
    for (int e = 0; e < N_TEST_EXPERTS; e++) {
        const size_t base_w1w2  = (size_t)e * W1_W2C_PORT_VECS;
        const size_t base_w1w2c = (size_t)e * W1_W2C_PORT_VECS;
        const size_t base_w3w2  = (size_t)e * W3_W2C_PORT_VECS;
        const size_t base_w3w2c = (size_t)e * W3_W2C_PORT_VECS;

        // ---- W1 rows split across a/b/c by row%3 ----
        // Total 683 groups; group rh covers W1 rows {3*rh, 3*rh+1, 3*rh+2}.
        // Row 2048 (the c-row of group 682) does not exist -> zero padding.
        for (int rh = 0; rh < W1_THIRD_ROWS; rh++) {
            const int row_a = 3*rh;
            const int row_b = 3*rh + 1;
            const int row_c = 3*rh + 2;
            for (int v = 0; v < HIDDEN_VECS; v++) {
                int16_vec_t va = Z, vb = Z, vc = Z;
                if (row_a < MOE_INTER) {
                    for (int k = 0; k < VEC_WIDTH; k++)
        ...
        for (int rh = 0; rh < W1_THIRD_ROWS; rh++) {
            ...
            for (int v = 0; v < HIDDEN_VECS; v++) {
                ...
                if (row_c < MOE_INTER) {
                    for (int k = 0; k < VEC_WIDTH; k++)
                        vc[k] = W3_all[e][row_c * HIDDEN + v * VEC_WIDTH + k];
                }
                W3_W2a_src[base_w3w2  + (size_t)rh * HIDDEN_VECS + v] = va;
                W3_W2b_src[base_w3w2  + (size_t)rh * HIDDEN_VECS + v] = vb;
                W3_W2c_src[base_w3w2c + (size_t)rh * HIDDEN_VECS + v] = vc;
            }
        }
        // ---- W2 first-half rows split across a/b/c by row%3 ----
        // First half = rows [0, HIDDEN/2). 1195 groups, last group's c-row is dummy.
        for (int rh = 0; rh < W2_SIXTH_ROWS; rh++) {
            ...
            for (int v = 0; v < INTER_VECS; v++) {
                ...
                if (row_c < half_bound) {
                    for (int k = 0; k < VEC_WIDTH; k++)
                        vc[k] = W2_all[e][row_c * MOE_INTER + v * VEC_WIDTH + k];
                }
                W1_W2a_src[base_w1w2  + W1_THIRD_VECS  + (size_t)rh * INTER_VECS + v] = va;
                W1_W2b_src[base_w1w2  + W1_THIRD_VECS  + (size_t)rh * INTER_VECS + v] = vb;
                W1_W2c_src[base_w1w2c + W1_THIRD_VECS  + (size_t)rh * INTER_VECS + v] = vc;
            }
        }
        // ---- W2 second-half rows split across a/b/c by row%3 ----
        // Second half = rows [HIDDEN/2, HIDDEN). 1195 groups, last group's c-row is dummy.
        for (int rh = 0; rh < W2_SIXTH_ROWS; rh++) {
            ...
            for (int v = 0; v < INTER_VECS; v++) {
                ...
                if (abs_c < HIDDEN) {
                    for (int k = 0; k < VEC_WIDTH; k++)
                        vc[k] = W2_all[e][abs_c * MOE_INTER + v * VEC_WIDTH + k];
                }
                W3_W2a_src[base_w3w2  + W3_THIRD_VECS  + (size_t)rh * INTER_VECS + v] = va;
                W3_W2b_src[base_w3w2  + W3_THIRD_VECS  + (size_t)rh * INTER_VECS + v] = vb;
                W3_W2c_src[base_w3w2c + W3_THIRD_VECS  + (size_t)rh * INTER_VECS + v] = vc;
            }
        }
    }
    ...
    std::vector<aligned_vector<int16_vec_t>> W1_W2c_slots(N_PARALLEL_SLOTS);
    ...
    std::vector<aligned_vector<int16_vec_t>> W3_W2c_slots(N_PARALLEL_SLOTS);
    ...
    // Two output buffers: y_mem_a holds the first HIDDEN/2 elements per token,
    // y_mem_b holds the second HIDDEN/2 elements per token.
    // Each has N * HIDDEN_VECS_FP/2 fp32 vectors.
    ...
    int64_t elapsed = tapa::invoke(
        ...
        tapa::read_only_mmaps<int16_vec_t, N_PARALLEL_SLOTS>(W1_W2c_slots),
        ...
        tapa::read_only_mmaps<int16_vec_t, N_PARALLEL_SLOTS>(W3_W2c_slots),
    ...
    // Compare: y_mem_a contains y[0..HIDDEN/2) per token, y_mem_b contains y[HIDDEN/2..HIDDEN).
    std::cout << "\n[3] Comparing..." << std::endl;
    double max_abs_err=0.0; int n_mismatch=0;
    const float tol=0.5f;
    const int half_vecs = HIDDEN_VECS_FP / 2;
    for (int t=0;t<N;t++) {
        // First half: from y_mem_a
