// ============================================================================
// MoE FPGA Testbench — batch_16b (INT16 weights, batched N=8)
// Mirrors moe_test_dec_spec.cpp but exercises the kernel with N>1 tokens per
// invocation. The reference model already supports N>1 (outer t-loop).
// ============================================================================
#include <iostream>
#include <vector>
#include <cmath>
...
int main(int argc, char** argv) {
    ...
    // Batched kernel: N tokens per invocation. N must be a positive multiple of 8.
    const int N = FLAGS_N;
    if (N <= 0 || (N % 8) != 0) {
    ...
    std::cout << "=== MoE FPGA Testbench (batch_16b — INT16 batched, N=" << N << ") ===" << std::endl;
    ...
    // Force routing to pick experts {0,1,2,3} for all tokens (kernel only has
    // N_TEST_EXPERTS=LOCAL_K=4 experts loaded; same set is reused across the
    // batch since each token may pick the same experts).
    ...
    // ------------------------------------------------------------------------
    // Symmetric zero-centered quantization for x — ONE global scale across all
    // N tokens of x (matches what the kernel expects: a single x_scale scalar).
    // ------------------------------------------------------------------------
    ...
    // Pack all N tokens of x into a single x_mem buffer of size N * HIDDEN_VECS.
    ...
    // ------------------------------------------------------------------------
    // Per-expert layout on each fused port (3-way row interleaving a/b/c).
    // ------------------------------------------------------------------------
    ...
    // Output buffers: each holds N * HIDDEN_VECS_FP/2 fp32 vectors.
