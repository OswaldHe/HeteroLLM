"""Minimal repro: dual GEMV with combined output (g * u, no silu)."""
import torch
import tilelang
...
def dual_gemv_combined(N, K, BLOCK_N, reduce_threads,
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(N, BLOCK_N),
            ...
            C[bn*BLOCK_N + tn] = T.Cast(dtype, C1_red[0] * C2_red[0])
...
ref2 = (A.float() @ B2.float().T)
ref  = (ref1 * ref2).to(torch.float16)

C = torch.empty(N, device="cuda", dtype=torch.float16)
kernel = dual_gemv_combined(N, K, 16, 64)
kernel(A, B1, B2, C)

err = (C.float() - ref.float()).abs().max().item()
print(f"err = {err:.5f}  ref_max={ref.abs().max().item():.5f}")
