// ============================================================================
// MoE FPGA Kernel — batch_16b_2ch (INT16, 2-channel a/b weight interleaving)
//
// Derived from moe_batch_16b.cpp by dropping the c-channel of W1_W2 and W3_W2.
// All other compute semantics (routing, dequant, swiglu, y accumulation) are
// preserved bit-exactly. Each parallel FFN slot now consumes 4 HBM weight
// ports (W1_W2a, W1_W2b, W3_W2a, W3_W2b) and the inner MAC drops from 3-way
// (a/b/c) to 2-way (a/b) emission per cycle.
// ============================================================================
...
// ============================================================================
// gate_compute — unchanged from baseline.
// ============================================================================
void gate_compute(
    const int   N,
...
// ============================================================================
// dispatch_core — unchanged from baseline.
// ============================================================================
void dispatch_core(
    const int   N,
    const int   local_expert_base,
...
// ============================================================================
// w1_w2_weight_loader / w3_w2_weight_loader — single function each, invoked
// twice per slot-set with different mmap bindings (a then b). Per
// `feedback_no_duplicate_functions`, do NOT clone these for a/b.
// ============================================================================
...
#pragma HLS loop_tripcount min=1 max=33
        ...
        if (trig < 0) { break; }
        const int base = trig * W1_W2_AB_PORT_VECS;
        for (int i_req = 0, i_resp = 0; i_resp < W1_W2_AB_PORT_VECS;) {
#pragma HLS pipeline II=1
            if ((i_req < W1_W2_AB_PORT_VECS) & !W_mem.read_addr.full()) {
...
#pragma HLS loop_tripcount min=1 max=33
        weight_trigger_t trig = trigger_in.read();
        if (trig < 0) { break; }
        const int base = trig * W3_W2_AB_PORT_VECS;
        for (int i_req = 0, i_resp = 0; i_resp < W3_W2_AB_PORT_VECS;) {
#pragma HLS pipeline II=1
            if ((i_req < W3_W2_AB_PORT_VECS) & !W_mem.read_addr.full()) {
                W_mem.read_addr.try_write(base + i_req); ++i_req;
            }
            if (!W_mem.read_data.empty()) {
                int16_vec_t tmp; W_mem.read_data.try_read(tmp);
...
// ============================================================================
// dequant_split — UNCHANGED from baseline.
// ============================================================================
void dequant_split(
    const float x_scale,
    tapa::istream<float>& dequant_in,
...
#pragma HLS pipeline II=1
        ...
        dequant_fwd.write(v);
        if (i == 0) {
            w1_dq_out.write(v * x_scale);
        } else if (i == 1) {
            w1_dq_out.write(v);
        } else if (i == 2) {
            w3_dq_out.write(v * x_scale);
        } else if (i == 3) {
            w3_dq_out.write(v);
        } else {
            w2_dq_a_out.write(v);
...
// ============================================================================
// ffn_x_split (2ch): emit 2 trigger streams per side (a and b) instead of 3.
// ============================================================================
void ffn_x_split(
...
// ============================================================================
// ffn_w1_w2_proj (2ch): 2-way MAC, emits float2_t to acc1.
// ============================================================================
void ffn_w1_w2_proj(
    tapa::istream<round_hdr_t>& hdr_in,
...
#pragma HLS array_partition variable=inter_buf cyclic factor=2 dim=2
    ap_uint<64> y_buf[TOKEN_LANE_PAIRS][2 * W2_AB_QUARTER_ROWS];
...
#pragma HLS unroll
                        ...
                        int lane = pos - n_vecs;
                        float2_t pair;
                        pair[0] = (float)sum_a[lane] * w1_scale + w1_shift;
                        pair[1] = (float)sum_b[lane] * w1_scale + w1_shift;
                        acc1_out.write(pair);
                    }
...
// ============================================================================
// ffn_w3_w2_proj (2ch): symmetric to ffn_w1_w2_proj.
// ============================================================================
void ffn_w3_w2_proj(
    tapa::istream<round_hdr_t>& hdr_in,
...
#pragma HLS unroll
                        ...
                        int lane = pos - n_vecs;
                        float2_t pair;
                        pair[0] = (float)sum_a[lane] * w3_scale + w3_shift;
                        pair[1] = (float)sum_b[lane] * w3_scale + w3_shift;
                        acc3_out.write(pair);
                    }
                    if (pos == row_iters - 1) { rh++; pos = 0; }
                    else { pos++; }
...
// ============================================================================
// ffn_swiglu (2ch): consume float2_t pairs from acc1/acc3, produce 2 outputs
// per rh into inter_lane_buf.
// ============================================================================
void ffn_swiglu(
    tapa::istream<round_hdr_t>&      hdr_in,
    ...
    float inter_lane_buf[TOKEN_LANES][2 * W1_AB_HALF_ROWS];
...
#pragma HLS pipeline II=1
                ...
                inter_lane_buf[lane][2*rh]     = va;
                inter_lane_buf[lane][2*rh + 1] = vb;
...
#pragma HLS unroll
        ...
        // Preserve scale-write-before-int16-write ordering: write all TOKEN_LANES
        // inter_scales before any inter_a/b half-vec.
        for (int lane = 0; lane < TOKEN_LANES; lane++) {
#pragma HLS pipeline
...
// ============================================================================
// y_accumulator_half — unchanged from baseline.
// ============================================================================
...
// ============================================================================
// y_writer_half — unchanged from baseline.
// ============================================================================
void y_writer_half(
    const int N,
...
void moe_fpga_top(
    ...
    // 2ch: only a/b triggers (c dropped).
    ...
    // 2ch: float2_t accumulators instead of float3_t.
    tapa::streams<float2_t, N_PARALLEL_SLOTS, 2> acc1_fifos("acc1");
    tapa::streams<float2_t, N_PARALLEL_SLOTS, 2> acc3_fifos("acc3");
    ...
    tapa::task()
        ...
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            ...
            x1_fifos, x3_fifos,
            trigger1a_fifos, trigger1b_fifos,
            trigger3a_fifos, trigger3b_fifos)
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            w1_w2_weight_loader, trigger1a_fifos, W1_W2a_expert_mem, W1_W2a_fifos)
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            w1_w2_weight_loader, trigger1b_fifos, W1_W2b_expert_mem, W1_W2b_fifos)
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            w3_w2_weight_loader, trigger3a_fifos, W3_W2a_expert_mem, W3_W2a_fifos)
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            w3_w2_weight_loader, trigger3b_fifos, W3_W2b_expert_mem, W3_W2b_fifos)
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            ffn_w1_w2_proj,
            hdr_to_w1,
            x1_fifos, W1_W2a_fifos, W1_W2b_fifos,
            w1_dq_fifos,
        ...
        .invoke<tapa::join, N_PARALLEL_SLOTS>(
            ffn_w3_w2_proj,
            hdr_to_w3,
            x3_fifos, W3_W2a_fifos, W3_W2b_fifos,
            w3_dq_fifos,
